...
def rand_row(data, target, num, total_num):
    row = np.random.choice(num, total_num)
    return data[row, :], target[row]

if __name__ == '__main__':
    
    iris = datasets.load_iris()
    data = iris.data[50:, :] 
    target = iris.target[50:] * 2 - 3
    data, target = rand_row(data, target, 100, 100)
